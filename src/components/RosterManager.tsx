...
export function RosterManager({ roster, onSave, onClose }: {
  ...
  return (
    <AccessibleModal open onClose={onClose} label="Team Roster" className="w-full max-w-sm rounded-3xl border border-pastelPink/20 bg-[rgba(25,22,40,0.95)] p-5 shadow-pastel-lg">
      <div className="mb-4 flex items-center justify-between">
        ...
        <button onClick={onClose} aria-label="Close roster" className="rounded-full p-1 text-inkDim hover:bg-pastelPink/10 hover:text-pink">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      <div className="mb-3 space-y-1.5">
        {players.map((p, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            />
            {p && (
              <button onClick={() => removePlayer(i)} aria-label={`Remove player ${p}`} className="text-inkDim/40 hover:text-pink transition-colors">
